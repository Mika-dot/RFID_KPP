...
def fetch_events(
    ...
    ORDER BY COALESCE(e.FinalizedAt, e.UpdatedAt, e.CompletedAt, e.LastSeen, e.FirstSeen) DESC, e.EventId DESC
    """
...
<body>
  <div class="page">
      ...
      <div class="card panel">
        <div class="panel-header">
          <div>
            ...
            <div class="panel-desc">Въезды, выезды, неопределенные и ожидающие допроверки по 30-минутным интервалам.</div>
          </div>
        </div>
      ...
      <div class="mini-grid">
        ...
        <div class="card panel" style="grid-column: 1 / -1;">
          ...
          <div class="logic-list">
            ...
            <div class="logic-item"><strong>3. Связь с 1С</strong> Событие связывается с 1С по полной метке или только по EPC; если данных пока нет, событие остается на повторной проверке.</div>
    ...
    <div class="card filters">
      ...
      <div class="field" style="grid-column: span 2;">
        <label>Поиск</label>
        <input id="fSearch" placeholder="Tag / SeriesNumber / EPC / TID / номер 1С / документ" />
      </div>
      <div class="actions">
  ...
  <script>
    ...
    function русСопоставление(v) {
      const m = { FULL_TAG: 'ПОЛНАЯ МЕТКА', EPC_ONLY: 'ТОЛЬКО EPC', NOT_FOUND: 'НЕ НАЙДЕНО' };
      return m[v] || v || '—';
    }

    function русТранспорт(v) {
      const m = { FORKLIFT: 'ПОГРУЗЧИК', HUMAN: 'ЧЕЛОВЕК', ALONE: 'САМА', UNKNOWN: 'НЕИЗВЕСТНО', forklift: 'погрузчик', human: 'человек', alone: 'сама', 'forklift+human': 'погрузчик+человек' };
      return m[v] || v || '—';
    }

    function русПричина(v) {
      const m = { TIMEOUT: 'ТАЙМАУТ', RECHECK: 'ПОВТОРНАЯ ПРОВЕРКА', MAX_DURATION: 'МАКСИМАЛЬНАЯ ДЛИТЕЛЬНОСТЬ' };
      return m[v] || v || '—';
    }
      ...
      timelineChart = new Chart(document.getElementById('timelineChart'), {
        ...
        data: {
          ...
          datasets: [
            ...
            { label: 'Ждут допроверки', data: dataPending, tension: 0.25, borderWidth: 2, borderDash: [6,4] },
          ]
        },
        options: {
    ...
    function renderMatchChart(summary) {
      ...
      matchChart = new Chart(document.getElementById('matchChart'), {
        type: 'doughnut',
        data: {
          labels: ['ПОЛНАЯ МЕТКА', 'ТОЛЬКО EPC', 'НЕ НАЙДЕНО'],
    ...
    async function openEvent(eventId) {
      ...
      document.getElementById('modalTitle').textContent = `Событие №${data.EventId} • ${русНаправление(data.FinalDirection)}`;
      document.getElementById('modalSub').textContent = `Ключ: ${data.EventKey} • Обновлено: ${fmtDate(data.UpdatedAt)}`;

      const evidence = JSON.stringify(data.EvidenceJsonParsed ?? data.EvidenceJson ?? {}, null, 2);
      const imageBlock = data.VideoEventId
        ? `<div class="img-box"><img src="/api/image/${data.VideoEventId}" alt="Снимок события" onerror="this.parentElement.innerHTML='Снимок недоступен'" /></div>`
        : `<div class="img-box">Снимок недоступен</div>`;
        ...
        <div>
          <div class="detail-card" style="margin-bottom:16px;">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item"><small>Закрыто</small><span>${fmtDate(data.CompletedAt)}</span></div>
              <div class="detail-item"><small>Финализировано</small><span>${fmtDate(data.FinalizedAt)}</span></div>
              ...
              <div class="detail-item"><small>Уверенность</small><span>${data.ConfidencePct || 0}%</span></div>
              <div class="detail-item"><small>Нужна допроверка</small><span>${data.NeedRecheck ? 'Да' : 'Нет'}</span></div>
              <div class="detail-item"><small>Тип транспорта</small><span>${русТранспорт(data.TransportMode)}</span></div>
              <div class="detail-item"><small>Предупреждения</small><span>${escapeHtml(data.WarningFlags || '—')}</span></div>
            </div>
          </div>
          <div class="detail-card" style="margin-bottom:16px;">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item"><small>Различных зон</small><span>${data.DistinctZoneCount ?? '—'}</span></div>
              ...
              <div class="detail-item"><small>Направление по RFID</small><span>${escapeHtml(русНаправление(data.RfidDirection))} (баллы ${data.RfidDirectionScore || 0})</span></div>
              <div class="detail-item"><small>RSSI ср/мин/макс</small><span>${data.AvgRSSI ?? '—'} / ${data.MinRSSI ?? '—'} / ${data.MaxRSSI ?? '—'}</span></div>
              <div class="detail-item"><small>Длительность</small><span>${data.DurationMs ?? '—'} ms</span></div>
            </div>
          </div>
          <div class="detail-card">
            <div class="detail-title">Подробности решения</div>
        ...
        <div>
          <div class="detail-card" style="margin-bottom:16px;">
            ...
            <div class="detail-grid" style="margin-top:12px;">
              <div class="detail-item"><small>Видео подтверждено</small><span>${data.VideoMatched ? 'Да' : 'Нет'}</span></div>
              <div class="detail-item"><small>Событие видео</small><span>${data.VideoEventId || '—'}</span></div>
              <div class="detail-item"><small>Время видео</small><span>${fmtDate(data.VideoTime)}</span></div>
              <div class="detail-item"><small>Направление видео</small><span>${escapeHtml(русНаправление(data.VideoDirection))}</span></div>
              <div class="detail-item"><small>Транспорт по видео</small><span>${escapeHtml(русТранспорт(data.VideoTransport))}</span></div>
              <div class="detail-item"><small>Баллы видео / Δt</small><span>${data.VideoScore || 0} / ${data.VideoTimeDeltaMs ?? '—'} ms</span></div>
            </div>
          </div>
          <div class="detail-card" style="margin-bottom:16px;">
            <div class="detail-title">СКУД</div>
            <div class="detail-grid">
              <div class="detail-item"><small>СКУД подтвержден</small><span>${data.SkudMatched ? 'Да' : 'Нет'}</span></div>
              <div class="detail-item"><small>Внешний номер СКУД</small><span>${data.SkudExternalId || '—'}</span></div>
              <div class="detail-item"><small>Время СКУД</small><span>${fmtDate(data.SkudTime)}</span></div>
              <div class="detail-item"><small>Направление СКУД</small><span>${escapeHtml(русНаправление(data.SkudDirection))}</span></div>
              <div class="detail-item"><small>Точка прохода</small><span>${escapeHtml(data.SkudGate || '—')}</span></div>
              <div class="detail-item"><small>Лицо/транспорт</small><span>${escapeHtml(data.SkudPerson || '—')}</span></div>
              <div class="detail-item"><small>Карта</small><span>${escapeHtml(data.SkudCard || '—')}</span></div>
              <div class="detail-item"><small>Баллы СКУД / Δt</small><span>${data.SkudScore || 0} / ${data.SkudTimeDeltaMs ?? '—'} ms</span></div>
            </div>
          </div>
          <div class="detail-card">
            <div class="detail-title">1С / повторная проверка</div>
            <div class="detail-grid">
              <div class="detail-item"><small>Номер 1С</small><span>${data.Task1CId || '—'}</span></div>
              <div class="detail-item"><small>Тип сопоставления с 1С</small><span>${escapeHtml(русСопоставление(data.TaskMatchType))}</span></div>
              <div class="detail-item"><small>Время 1С</small><span>${fmtDate(data.Task1CDt)}</span></div>
              <div class="detail-item"><small>Серийный номер</small><span>${escapeHtml(data.Task1CSeriesNumber || '—')}</span></div>
              <div class="detail-item"><small>Документ 1С</small><span>${escapeHtml(data.Task1CDocIds || '—')}</span></div>
              <div class="detail-item"><small>Следующая проверка</small><span>${fmtDate(data.NextRecheckAt)}</span></div>
              <div class="detail-item"><small>Число проверок</small><span>${data.RecheckCount || 0}</span></div>
              <div class="detail-item"><small>Последняя проверка</small><span>${fmtDate(data.LastRecheckAt)}</span></div>
              <div class="detail-item"><small>Причина закрытия сессии</small><span>${escapeHtml(русПричина(data.SessionCloseReason))}</span></div>
            </div>
          </div>
        </div>
      `;
...
if __name__ == "__main__":
    print("\n" + "█" * 100)
    print("🌐 КПП • ВЕБ-ПАНЕЛЬ по KPP_ReelEvents")
    print("█" * 100)
