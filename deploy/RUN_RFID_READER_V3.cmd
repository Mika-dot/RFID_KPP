...
title 1 RFID Reader v3.4.7 Continuous Recovery
cd /d "%~dp0.."
set "ROOT=%CD%"
...
if not exist "%ROOT%\deploy\monitored_rfid_recovery_v2.py" goto :bad_monitor
...
"%PY32%" -u "%ROOT%\deploy\run_service.py" --service "Perimeter.RfidReader" --script "%ROOT%\deploy\monitored_rfid_recovery_v2.py"
set "RC=%ERRORLEVEL%"
...
:bad_monitor
echo [FATAL] Missing deploy\monitored_rfid_recovery_v2.py
goto :fatal
