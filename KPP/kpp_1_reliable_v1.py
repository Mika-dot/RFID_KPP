...
class Config:
    ...
    TASK_TABLE = os.getenv("KPP_TASK_TABLE", "dbo.Rfid1С")
    ...
    # --- 1С и переобогащение ---
    TASK_LOOKBACK_HOURS = int(os.getenv("KPP_TASK_LOOKBACK_HOURS", "168"))
...
class KPPMonitorReliable:
    ...
    def _conn_1c(self) -> pyodbc.Connection:
        return pyodbc.connect(Config.ONEC_CONN_STR, autocommit=False)
...
WHERE Dt >= DATEADD(hour, -?, GETDATE())
ORDER BY Dt DESC;
"""
        try:
            by_full: Dict[str, List[Rfid1CTask]] = {}
            by_epc: Dict[str, List[Rfid1CTask]] = {}
            with self._conn_1c() as conn:
                cur = conn.cursor()
                ...
                for row in cur.fetchall():
                    task = Rfid1CTask(
                        ...
                        doc_ids=str(row[3]) if row[3] else "",
                    )
                    if not task.tag:
                        continue
                    by_full.setdefault(task.tag, []).append(task)
                    if task.epc:
                        by_epc.setdefault(task.epc, []).append(task)

            self.tasks_by_full_tag = by_full
            self.tasks_by_epc = by_epc
            self.last_task_reload_at = datetime.now()
            self._log(
                f"Задачи 1С загружены: {sum(len(v) for v in by_full.values())} записей, уникальных меток: {len(by_full)}",
                "OK",
            )
            return len(by_full)
        except Exception as exc:
...
"""
    ...
    def find_best_task(self, session: TagSession) -> Tuple[Optional[Rfid1CTask], str, List[str]]:
        ...
        if exact:
            task = min(exact, key=lambda t: abs((t.dt - session.first_seen).total_seconds()))
            if len(exact) > 1:
        ...
        if epc_matches:
            task = min(epc_matches, key=lambda t: abs((t.dt - session.first_seen).total_seconds()))
...
"""
    ...
    def _upsert_event(
        ...
        data = {
            ...
            "VideoScore": video.score if video else None,
            ...
            "SkudScore": skud.score if skud else None,
...
"""
    ...
    def maybe_print_status(self) -> None:
        if self.last_status_at is None or (datetime.now() - self.last_status_at).total_seconds() >= Config.STATUS_INTERVAL_SEC:
            ...
            self._log(
                f"Статус: задач={sum(len(v) for v in self.tasks_by_full_tag.values())} | активных RFID-сессий={len(self.active_sessions)} | last_rfid_id={self.last_rfid_id} | сохранено={self.saved_reports}",
                "INFO",
            )
