...
2. Warehouse подключён как источник склада.
3. Если КПП-событие есть — оно дополняется складом.
4. Если катушка появилась на складе, но КПП её не увидел — создаётся событие WAREHOUSE_ONLY в KPP_ReelEvents.
```
